...
if errorlevel 0 (
    %msbuild% SuperSocket.2013.sln /p:Configuration=Debug /t:Clean;Rebuild /p:OutputPath=..\..\bin\Debug
	FOR /F "tokens=*" %%G IN ('DIR /B /AD /S obj') DO RMDIR /S /Q "%%G"

	%msbuild% SuperSocket.2013.sln /p:Configuration=Release /t:Clean;Rebuild /p:OutputPath=..\..\bin\Release
	FOR /F "tokens=*" %%G IN ('DIR /B /AD /S obj') DO RMDIR /S /Q "%%G"
)
